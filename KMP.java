...
//这里面又有细节了，当前这位不匹配，那么我们要怎么进行回溯呢？直接找当前位是行不同的，因为在构建的时候，我们给prefix赋的值是相同值那位的下一位，
//也就是说，当前位不匹配的话，你要先把j给减1， 然后找减1后的prefix值，再赋值给j， 就变成了 j = prefix[j - 1]
//当j是0的时候就没办法了，只能让i加1，重头开始匹配了
//
//看到这里，你可能会有个疑惑，i直接加1没问题吗，万一在i和j刚开始匹配的时候，正好i的下一位和当时的j完全匹配的，
//其实这种情况是不存在的，因为当我们没有匹配上的时候，我们做的回溯相当于把pat数组右移，我都玩命右移没找到，那其实就不存在这种情况了
